...
| 2 | 1.5× | 11 | `LayoutSiege.tsx` | **The Siege** *(eased 2026-07-14, playtest-gated)* | 32 | **12** | 5 (keep) | **none** |
| 3 | 2.0× | 14 | `LayoutPortcullis.tsx` | **The Portcullis** *(lock reworked 2026-07-14, playtest-gated)* | 30 | **7** | 4 (the bar) | **none** |
...
| 7 | 4.0× | 15 | `LayoutEyrie.tsx` | **The Eyrie** *(grip+weave 2026-07-14, playtest-gated)* | 29 | **9** | 5 (the apex) | 7 open + 2 talons |

Shelved (not in rotation): `Layout9.tsx` "Cross of Clans" + `Layout7.tsx` "The Stronghold" (cut by the 2026-07-02 audit, below), `LayoutHourglass.tsx` "The Hourglass" (cut 2026-07-09 — the Last Grain choke stalled runs) + its replacement `LayoutFloodgates.tsx` "The Floodgates" (cut 2026-07-10, below), `LayoutPeaks.tsx` "The Mythic Peaks" classic board (cut 2026-07-10 — the other half of the funnel pair), `Layout2.tsx` (unnamed piles), `Layout3.tsx` "Fortress" (36), `Layout4c.tsx` "Reverse Tripeaks" (36), `Layout6.tsx` "Colosseum" (32).

> **2026-07-14 — The Eyrie reworked: THE GRIP AND THE WEAVE (Task 3).** Finding: the finale played itself — two independent wing cascades that contested nothing, talons as pure free fuel, inner mids (5/6/8/9) blocking nothing; no "spend it or hold it" anywhere on the 4.0× field. Two changes, shape untouched: **the grip** — each talon now pins its wing's EDGE FEATHER (outermost mid: 3←12,27 · 11←19,28; the column talon → leading covert → edge mid rises outward), so spending a talon is fuel AND the key to that wing's pinion route, holding it is chain insurance, and both grips must open before the apex; **the weave** — the head lifts only once the mantling wings part (7←6,8,15,16 — the inner mids flanking it on its own row), so the center ascent runs through both wings and inner mids 6/8 get jobs (5/9 stay as feather-pocket fuel). A stronger variant also weaving the perch through the inner coverts (15←14,22,23) measured first-5 1.7 — a drought on the finale — and was cut. Shipped numbers: dead 49%→48% (set-best), burst 17.4%, first-5 2.6 (was 2.7), last-5 2.2→2.4 (the finale now climaxes; only the Siege ends livelier), depth 5, keystone-23 double-pop intact. Machine-verified.
>
> **2026-07-14 — The Portcullis lock reworked: THE TWO HINGES (Task 3).** Owner-confirmed finding: the old endgame was opaque (brackets gated on the heart-stone four rows away at the top rail, the bar on posts at the meeting row's far edges — locked cards with no visible cover) and then anticlimactic (all four released at once, same forced sequence every run). New lock, every dependency local: hinge posts 28/29 render at the bracket row as the gate's JAMBS (still open from move 1); a bracket tears free when its rising pair is out AND its own post is pulled (24←20,21,28 · 25←22,23,29); the bar drops only when both brackets tear (26,27←24,25 — the gate falls as one beam). The posts become fuel AND per-side keys — when to spend each is the endgame. Heart-stone 4 loses its cross-board lock role. Dead 57%→54%, burst 24.5% + maxBurst 4 (both still set-best), first-5 5.1 unchanged, depth 5→4; last-5 2.2→1.7 (the old number was the simultaneous collapse counting as a climax). Machine-verified.
>
> **2026-07-14 — The Siege eased (Task 3).** Owner verdict: the L2 opening still too slow after the 07-09 rework (first-5 2.9, burst 11.5% — the set's worst). The rail pockets now fall with their CORNER guard alone (1←0 · 3←4 · 12←11 · 14←15, was two guards each) — every corner guard is a 2-card burst (pocket + breach stone), one rule taught four times; the center guards 2/13 become held fuel. First-5 3.8, burst 16.4%, last-5 2.6 (keep climax intact, still set-best); dead 49%→54%, inside the shipped band. Double symmetry (L-R and N-S) holds; machine-verified.
>
...
> **2026-07-10 — The Eyrie (L7, 4.0×, 29c, 9 open)** replaces the classic tri-peaks finale (the other half of the funnel pair — 10→8→6→3→1 was the Floodgates' 8→7→3→6→5→1 flipped). The war eagle mantling on the summit — the eyrie IS the top of the Mythic Peaks, so the finale keeps the namesake's summit story with a shape all its own: the **wings cascade OUTWARD** (body → coverts → 4-wide mids, band widening 1.33 — the set's only diverging flow; every other board converges), the center runs a short ascent (perch → head), and the campaign ends on **one fused apex card blocked by both pinions AND the head** (depth 5) — the old three summits reborn as a single crowning moment. Two open talon pockets at the crag corners are held fuel. Machine-verified clearable + mirror-isomorphic; cascade 1.35 (mid-band). Dead 49% / burst 17.7% / 0.69 opened per capture (the funnel: 51% / 13.1% / 0.64), first-5 2.7, last-5 2.2. Identity: **ride both wings while the perch trio is live, and time the apex so the crown lands on a banner.** Owner feel-gate before release.
