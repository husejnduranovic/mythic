...
- **Consistency debt created for next slices** (the previously-built screens still use the *old* emoji/white-overlay grammar):
  - **Armory `MiniBack`/`MiniBounty`/stage diorama** (`Armory.tsx`, commit 7) — still emoji crests + white-overlay frames. To stay "true miniature", port to `BACK_STYLES` accents + `<Sigil>`. **Highest priority** — it's the screen that sells these backs. (`BACK_ICONS` is already gone; `MiniBack` reads `item.icon`/`item.accent`, so the data it needs is local.)
  - **Game chrome** (`Game.tsx`): Free Draw card (§4.2.1) and `openCardGlow`/deck badge still pre-token; fold into the same pass that does §4.2.1–4.2.7.
  - **Battlefield** center beast watermark (`game/Battlefield.tsx`) is still emoji (§4.2.6 kept it) — when that lands, use the `<Sigil>` dragon for one engraved language end-to-end.
  - **Dormant hint system**: `hinted` prop + `hintGlow` style exist on `Card` but `showHints` is permanently `false` in `Game.tsx` (never set true) and no layout passes `hinted` through to the wrapper — the glow is unreachable. Either wire it or delete it in the Game slice; left untouched here.
