...
> - **The exhale itemized**: Breath deals in COMBAT/TIME/DECK/PERFECT/UNBROKEN source chips — discipline becomes visible money.
> - **First-session arc**: R2 coach marks (3 parchment marks, first free battle, clock frozen) + R5 First Victory (one-time overlay → Armory, veteran-gated) + §10 A-1 share button (record/flawless-aware message).
> - **Return hooks**: THE SHADOW named at pre-battle; **R4 Ember Ward** (one missed day/week forgiven) + a real streak-killing bug fixed (`getYesterdayString` was UTC vs local `getTodayString` — east-of-UTC players lost streaks just after midnight; now exact local-day diff).
> - Fix slice: milestone banner captures its combo at show time (CHAIN BROKEN read live x0), First Victory games-played gate for updating veterans.
>
> **Day 2 (2026-07-02) — the rethink pass** (rows 33–35 below + `GAMEPLAY.md` §1.5):
> - **The Citadel** replaced the asymmetric Warfront (owner constraint: symmetric shapes only) — the Breach kept, flanks rebuilt as identical mirrored bastions; blocking graph verified mirror-isomorphic. Playtest gate stands.
> - **The felt got light** — accent-tinted breathing table light, per-battlefield giant crests (fixed the id-mismatch bug that made all 8 fields render identically), forged corner brackets.
> - **Between-levels rebuilt as "The Breath"** — exhale → flip-reveal of the next field → ghost line; the two-column report is gone.
> - **Run systems**: Free Draw banking (cap 2), the ghost (race your best run's per-field pace), Flawless Conquest accolade. Plus reset/stale-state bug fixes (restart/rematch leaks, atomic banner banking).
>
> **Day 1 (2026-07-01)** — shipped rows 27–32 + the gameplay work logged in `GAMEPLAY.md`:
> - **Game board** — the marquee war-table recomposition (27): carved wells, current-card dais, banner-progress combo meter, native fuse, unfurling milestone banner.
> - **Cards** — tiered vanquish shockwave + flip settle (28); Layout7's invisible-bounty bug fixed.
> - **Scoring v2 — "Spoils, Banners & Bounties"** (`GAMEPLAY.md §1.5`): capped 100× curve, banked banner milestones (one aligned ladder — fixes the freeze/banner defect), bounty ×3-at-tier, seeded bounty placement. **`allTimeScores` wipe decision pending at 1.4 release** (docs stamped `scoringV: 2`).
> - **Layouts**: **The Warfront** (new, level 5, asymmetric/no-base) + Dragon's Spine interlocked base — **both owner-playtest-gated**.
> - **Moments**: game-over count-up/stamp/rank-punch + Banners ledger; between-levels ignition + named battlefields; pre-battle road deal.
> - **Armory**: skill gates (×12/×20/×28 combo, 1M spoils) + the wild-style inventory deleted (Findings #1 → resolved: deleted).
>
> **Still flagged:** **Lounge** & **Guide** — on the design system (24/23/25/26) but not yet given a distinctive composition identity; revisit with device screenshots.
> **Daily-tab bug:** the keep-podium-mounted fallback is now committed (`a3c266d`) — needs on-device confirm.
> **Carryover:** shared-component migration sweep (extract `TabBar`; menu screens → `GoldButton`/`honor.tsx`); `migrateArmoryIfNeeded` retirement question.
...
| Commit | Scope | Status |
|---|---|---|
| _(commit 45)_ | **The stakes at the door** (MARKET.md A-5). Auth sign-in gains the €100-prize pill in Home's grammar (trophy + LIVE chip) — a fresh install reads the stakes before the first battle. | ✅ built (device-verify) |
| _(commit 44)_ | **GoldButton: press weight + the real disabled state.** Every primary CTA app-wide presses with the dais-punch (dip → overshoot spring, event-driven native transforms). The §3-spec disabled state (washed gold, hairline, no shadow, inert) finally implemented; Arena-lobby Start / Auth ENTER / Lounge JOIN migrate off their fake opacity-disabled styles (dead keys removed). `minWidth` moved to the outer wrapper so caller layout overrides keep working. | ✅ built (device-verify) |
| _(commit 43)_ | **The forge moment** (`Armory.tsx` + `SoundService.playForge`). Equipping strikes the stage: first-banner sting + heavy haptic, a gold flash spiking over the diorama, a FORGED stamp punching in back-eased and fading while the gear re-deals. Fires only on real changes; idle at 0. | ✅ built (device-verify) |
| _(commit 42)_ | **Home energy + the shell enters** (the menu-side of the moments brief). Home: layered table-light pool replaces the hard glow blob (breathes on the existing glowPulse), grounding shelf-shadow counter-breathes under the levitating fan, dais-punch press weight on Enter Battle + both mode cards, streak ember flickers / spent Ward counter-breathes (rides dotPulse) — zero new idle loops. Shell: `src/ui/ScreenEnter.tsx`, a 200ms fade+rise keyed per screen in App's map — the app previously hard-cut between composed rooms. | ✅ built (device-verify) |
| _(commit 41)_ | **Fix slice** — milestone banner captures `comboAt` at show time (footer hidden <x2; CHAIN BROKEN had read live x0 after the same-press reset); First Victory adds a games-played gate so updating veterans get the flag silently, never the overlay. | ✅ built (device-verify) |
| _(commit 40)_ | **Ember Ward + streak-day fix** (§6 R4). One missed day per rolling week forgiven: 2-day gap with the ward ready continues the streak and stamps `emberWardUsedAt` (once/7d); multi-day gaps still reset; Home streak chip grows an ember shield for a day after it fires (`useUserStats` → `emberWarded`). **Bug found in the same block:** `getYesterdayString` was UTC (`toISOString`) while `getTodayString` was local-parts — east-of-UTC players (the DACH/Balkan core) playing just after local midnight compared against the wrong yesterday and had streaks wrongly reset, plausibly capping the observed streaks at 5. Streak logic now uses an exact local-day diff. | ✅ built (device-verify) |
| _(commit 39)_ | **The shadow named at the gate.** Pre-battle renders THE SHADOW · {best-run total} (ghost glyph, steel) under the road ahead — the race against your best starts when you decide to play, not three fields in. Normal mode only; daily keeps the sealed quest card. | ✅ built (device-verify) |
| _(commit 38)_ | **Share the spoils + First Victory** (§10 A-1, §6 R5). Game-over ghost row fires the system share sheet, message led by the run's best claim (record > flawless > spoils) + store link — the record moment finally carries. First Victory: first-ever completed solo battle gets a one-time overlay (FIRST VICTORY stamp → "the Armory is open" → CHOOSE YOUR COLORS routes into the Armory via new `onGoArmory`); arena keeps its rankings moment. | ✅ built (device-verify) |
| _(commit 37)_ | **First-battle coach marks** (§6 R2). Three parchment marks on the first-ever free battle: the match rule when the board opens, the chain at first combo ≥2, the Free Draw right after — each pinned above what it teaches, clock frozen while up, any tap dismisses, `@mythic_seen_coachmarks` written on first dismissal. Parchment = the nameplate grammar (a note pinned to the war table). Mounted only while showing. | ✅ built (device-verify) |
| _(commit 36b)_ | **The exhale itemized.** The Breath's count-up now deals in staggered source chips — COMBAT / TIME / DECK / PERFECT / UNBROKEN, zeros skipped — and an unbroken field re-stamps the header UNBROKEN CONQUEST in goldBright. The scoring's decision-rewards become visible money. Display-only. | ✅ built (device-verify) |
| _(commit 36a)_ | **Capture weight.** Every capture/draw thunks the incoming current card onto the dais (dip 0.9 → 1.06 → 1, one shared value on the dais wrapper — the field never re-renders); a chain worth ≥1 banner dying to a deck draw stamps steel CHAIN BROKEN — x{n} FELL · BANNERS HOLD; the orphaned 10s `playTimeWarning` haptic is wired into the fuse. | ✅ built (device-verify) |
| _(commit 36)_ | **The clear-hold + UNBROKEN + BoardBurst** (`GAMEPLAY.md` §1.5.4–6). The eaten perfect-clear moment fixed (banner + screen swap fired in one synchronous block — the banner never drew); won fields hold the board 950/1500ms with the clock stopped and inputs guarded. UNBROKEN: field consumed by one unbroken chain → second perfect-clear bonus (**scoring change**, 100k×mult total), goldBright triple-ring detonation, layered `playUnbroken` fanfare, 1350ms banner, earned-only game-over row. `BoardBurst` also fires on banner plants ≥20 (big ≥28) — one-shot, ~700ms mount, one shared value, zero per-card cost. Milestone holds tiered; bounty+banner same-tap fade bug fixed. | ✅ built (device-verify) |
| _(commit 35)_ | **Flawless Conquest** — the apex run named: all fields perfect → goldBright title + star medallion + FLAWLESS card badge (record still outranks); ledger reads quality over volume (Perfect Fields x/6 replaces raw beasts-captured). | ✅ built (device-verify) |
| _(commit 34)_ | **Between-levels rebuilt — "The Breath"** (the flagged Opus-flavored screen, redesigned from purpose, not restyled). Three beats on one center axis: **exhale** (outcome stamps, field spoils count into the campaign total), **anticipation** (the next battlefield deals face-down and **flips** to reveal name/mult/beasts/time — VICTORY card on the final field), **onward** (the **ghost line**: ahead/behind your best run at this point with comeback framing; banked-draw note; march dots; compact Glory chip; CTA). Instantly skippable; arena keeps standings beside a smaller reveal. The old two-column honor-card report is deleted. | ✅ built (device-verify) |
| _(commit 33)_ | **The felt gets light** (`game/Battlefield.tsx` rewrite — the "does it feel like the icon?" gap). Stacked accent-tinted **table-light pool** (hot warm core → dark edges) breathing on one shared native loop; **giant per-battlefield crest** watermarked under the cards (volcanic=fire, coastal=waves, royal=crown…); forged **corner brackets** in the field's accent metal; corner shades + stronger vignettes; chains/torch clutter gone. **Bug fixed:** the old theme table was keyed by ids that never matched the real battlefield ids — all eight Armory fields had rendered identical decor since the rename. Memoized, ~45 static views, zero per-frame JS. | ✅ built (device-verify) |
| _(commit 32)_ | **Guide teaches with the real pieces** (§4.10 — the flagged "distinctive identity" revisit). The four card-lesson slides swap the abstract icon-in-a-ring for a **live tableau of actual `Card` components**: a 7♦ beside an 8♥ for matching, an ascending 5-6-7 fan for combos, the tooled-leather deck back for the reset, and a **plain back next to a bounty back** for "spot the special card" — always current with the live render, never a drawing of it. Halo widens into a glow pool under the fan; non-card slides keep the ring medallion. **Copy updated to scoring v2**: combo slide teaches banner planting (+ ladder in the example line), deck slide notes banked banners survive, bounty slide teaches ×3-at-tier late-chain routing. **Lounge stays flagged** — three passes shipped already (24/25/26); a fourth without device screenshots has poor expected value (the §4.6 lesson: blind token passes read flat). Revisit with screenshots. | ✅ built (device-verify) |
| _(commit 31)_ | **Between-levels & pre-battle beats** (§4.3/§4.2 follow-through). The next march card **ignites** (back-eased pop, 420ms after the result card lands) instead of statically glowing; the next-battlefield preview **names the field** in Cinzel (`LEVEL_CONFIG` gained per-level names — Battlements → Dragon's Spine); pre-battle's six road cards **deal in on 70ms beats**. Paused / Already-Played / Quit-Confirm re-reviewed and deliberately unchanged — slice 17's composition holds. | ✅ built (device-verify) |
| _(commit 30)_ | **Game-over becomes a moment** (§4.4 follow-through). The spoils **count up** into the sealed card (26 eased steps, ~900ms, starting once the card lands); the outcome title **stamps** on (back-eased 1.6→1); the async rank **punches** out of its shimmer the frame it resolves; the chronicle gains a **BANNERS PLANTED** ledger row (run-scoped counter in Game.tsx — celebrates the scoring-v2 mechanic). | ✅ built (device-verify) |
| _(commit 29)_ | **Armory skill gates + wild inventory deleted** (GAMEPLAY §2.9 G3 / Findings #1 resolved). Four grind unlocks re-pointed at mastery: Eagle Pennant ×12 combo, Royal Banner ×20, Obsidian ×28 (lifetime best combo, `@mythic_best_combo_ever`), Diamond 1M spoils in one battle (local score history) — `ArmoryItem` gains `comboReq`/`scoreReq`, the stage forge-ladder shows the right gauge per gate (streak=ember fire, combo=banner flag, spoils=sack, battles=swords) and skill-locked tiles get gold flag/sack chips + gold req text. **Wilds deleted**: `WILD_STYLES` + `WILD_STYLE_CONFIG` + `wildStyle` off `ThemeConfig`/`getSelectedTheme`/Game state/storage keys — six unreachable items (incl. the 42-day Inferno Bolt) stop being dishonest inventory; old stored values orphan harmlessly. Note: combo/spoils gate progress is device-local (like the score history) — a reinstall re-earns them. | ✅ built (device-verify) |
| _(commit 28)_ | **Card feel — tiered vanquish + flip settle** (§4.2.8 follow-through). The capture now escalates with the chain: combo tiers ≥8 / ≥16 / ≥24 recoil harder (pop 1.12→1.15→1.18) and throw a **shockwave ring** (gold → ember → red) that expands and dissolves with the fall — one extra view, mounted only on the one falling card for its 230ms. Tier reaches the card through a **ref-in-context** (`VanquishTierContext`, stable value object) so combo changes never re-render the 27 other memoized field cards. The **flip reveal lands with a 7° over-rotation settle** (240ms) instead of easing to a stop. Cleanup: dead `pending`/`pendingIndex` machinery stripped from Card + all nine layouts (vestige of removed wild placement), dead `bountyConfig` prop dropped; **bug fix: Layout7/Stronghold never threaded `bountyIndices`** — level 3's bounties scored but rendered as plain cards; now visible. | ✅ built (device-verify) |
| _(commit 27)_ | **Live board recomposed — "The War Table"** (the paused Fable-level pass; §4.2 marquee). The bar stops reading as three bordered widgets: stations are now **carved wells** set into the wood (dark inset, shadowed top lip, catch-light bottom edge — no gold outlines) and the **current card(s) stand on a breathing dais** that rises 7px past the battlement line (content-hugging inlay + engraved frame; one native-driver opacity loop total — the board's only idle animation). **Treasury** flashes gold the instant spoils land (Reanimated overlay synced to the score pulse). **Combo gauge** rebuilt as a banner meter: `x{combo}` + a fill that tracks progress to the **next milestone flag** (`flag-variant` + target number; checkered flag past 32) — the push-your-luck state is now readable at a glance, and the tier-jumping font sizes that jittered the row are gone (fixed 16, tier lives in color+glow+pulse). **Timer fuse** converted from a JS-thread width animation (the one per-second JS animation on the live board) to a native-driver left-pinned scaleX; gauge box dropped for the well. **Milestone popup** is now a war banner: colored rod along the top edge, cloth **unfurls** (scaleY 0.5→1 + pop — the old entrance was an unanimated snap), and it carries an optional bank sub-line (wired by scoring v2). Back ✕ text glyph → `close` Icon; dead `freezePulse` value deleted. Perf contract: field untouched, everything native-driver, +1 idle loop (dais) −1 JS-per-second (fuse). | ✅ built (device-verify) |
...
| _(commit 25)_ | **Armory lock visibility** (device-test: locked items looked selectable because the lock chip was too small). The `RackTile` lock medallion — shared by all four racks (backs / fields / bounty / table) — grew from 22→32 px with the glyph 11→18 and a drop shadow so it clearly sits on top of the preview, and the dimmed-preview opacity dropped 0.35→0.3. A locked item now unmistakably reads as locked rather than choosable. | ✅ built (device-verify) |
| _(commit 24)_ | **Lounge recomposition — "Venue Hall"** (`LoungeScreen.tsx` — untouched until now; §4.11). The Hookah-Lounge weekly tournament recomposed as a venue-scaled Hall of Glory, reusing the shared chassis directly: **joined** = landscape two-column with the player's standing rendered through `HonorCard` (place-metal trim, `#rank` corner index, Cinzel YOUR-SPOILS hero number, deal-in + levitate) on the left, and the week's warriors as a muster roll on the right (rank + place/tier ring via `TIER`/`withAlpha`, dotted leaders, combo pills, place metals for the top 3, parchment YOU chip, cascade-in) + prize strip; Enter Battle + Return via `GoldButton`/ReturnToCastle, Leave as a corner danger chip. **Join** = ornate header + a Cinzel code-input panel + JOIN `GoldButton`. Off-palette blue `#4FC3F7` → **sage** venue accent (Home's lounge dot); the bg `🏛`/`🏆` emoji watermarks removed; `useWindowDimensions` + safe-area insets. Service/data/flow unchanged. | ✅ built (device-verify) |
| _(commit 23)_ | **Guide / How-to-play recomposition** (`Introscreen.tsx` — untouched until now; §4.10). Brought onto the design system: token palette + Cinzel (splash title `font.display`; HOW-TO-PLAY header + slide titles), MCI tutorial icons in a breathing ring medallion reusing the **in-game vocabulary** (`restore`=Free Draw, `sack`=Bounty, `lightning-bolt`=Glory Hunt, `image-filter-hdr`=Battlefields, `trophy-variant`=Prizes, `fire`=Combo), shared `GoldButton` for Next/Enter, token dots + ghost Back with chevrons. The four card-face beasts on the first-run splash stay emoji (§9, illustration). Slide copy / phase flow / AsyncStorage + `hasSeenIntro` unchanged. | ✅ built (device-verify) |
| _(commit 22)_ | **Card-removal animation elevated — "vanquish"** (device-test: explore a more satisfying capture). The flat drop-and-fade (fall +80 → shrink → fade) becomes a **recoil pop → shrink-and-rise dissolve**: a quick scale-up to 1.12 (60ms) that punches up the combo feel on each capture, then the card shrinks (→0.45) and lifts (translateY −28) with a slight spin as it fades (~245 ms total). Entirely UI-thread (Reanimated `withSequence`/`withDelay` over transform + opacity), one card at a time — same perf profile as before (only the captured card animates, no idle loops), just snappier and more rewarding. The flip/reveal is unchanged. | ✅ built (device-verify) |
| _(commit 21)_ | **Cinzel hero numbers across the game screen-states** (device-test: game-over / between-levels fonts didn't match Home & the rest). The titles were already Cinzel; the **hero numbers** now are too — game-over Spoils score + resolved rank `#N`; between-levels TOTAL SPOILS + next-battlefield preview stats + arena countdown; already-played score; paused / quit spoils-at-stake — all on `font.display`/`font.heading`, matching the live-board Spoils (slice 20) and the card values. Secondary Glory-Hunt button labels → Cinzel heading too. Tiny (7–8px) letterspaced overlines stay system (Cinzel is illegible that small). Font-only. | ✅ built (device-verify) |
| _(commit 20)_ | **Spoils & Timer redesigned + board numerals on Cinzel** (device-test: they read like a flat calculator). **Timer** (`Timer.tsx`) → an engraved war-gauge: a `timer-sand` hourglass glyph + the time in **Cinzel** (`font.display`) with a state-colour glow (gold→ember→crimson; frost + `snowflake` when frozen), over a thicker framed fuse bar — the flat dark box is gone. **Spoils** (`Game.tsx` treasury) → a mini honor-card plaque: engraved inset frame + the `sack` crest in a dark medallion well + SPOILS in Cinzel `font.heading` + the score in Cinzel `font.display` with glow. The board's other live numerals also moved to Cinzel so they match the (already-Cinzel) card values beside them — combo value, points-popup, milestone banner. Font-only / static changes; the score/combo pulses (native-driver) are unchanged and nothing new runs per frame. | ✅ built (device-verify) |
| _(commit 19)_ | **Shared `GoldButton` (`src/ui/GoldButton.tsx`) — board CTAs match Home** (P1-1; device-test feedback: board buttons looked plainer than Home's). Lifted Home's "Enter Battle" craft into a reusable button — gold fill + **engraved inner pinstripe** + **Cinzel (`font.heading`) label** + coloured shadow — with `primary`/`ember`/`danger` variants. Adopted by every board primary action: Game-over (Battle Again / Return), Between-levels (Next Battle / Claim Victory / Begin Glory Hunt→ember), Pre-battle (Enter Battle / Glory→ember), Paused (Resume), Quit (Keep Fighting primary + Retreat danger). Per-screen ad-hoc gold-button copies deleted. | ✅ built (device-verify) |
| _(commit 18)_ | **War-bar HUD composed into war-table stations** (the live board). The card *field* is owner-locked (positioning confirmed across devices), so the HUD strip itself is what gets restructured: the bare SPOILS text → a framed **treasury cartouche** (`sack` glyph + label + score plaque set into the table), and the timer/combo → a matching framed **gauge station**, so the bottom ledge reads as a segmented war table instead of three floating text groups. Perf-safe within the fixed bar height — ~2 extra static Views, no new per-frame work, the existing score/combo pulses (native-driver Reanimated) unchanged, the ~28-card field untouched. | ✅ built (device-verify) |
| _(commit 17)_ | **Utility screen-states composed to grammar** — Paused, Already-Played, Quit-Confirm. **Paused** → a centered "war camp" panel (pause medallion in a gold ring, Cinzel title, SPOILS-HELD cartouche, Resume primary + Restart/Castle ghost), scale-in. **AlreadyPlayed** → today's daily run as a sealed honor card (`check-decagram` medallion, TODAY'S SPOILS hero number, "RETURNS AT MIDNIGHT"), deal-in + levitate. **QuitConfirm** → composed modal (skull-in-crimson-ring medallion, Cinzel "RETREAT?", spoils-at-stake cartouche, Keep Fighting primary + danger Retreat), scale-in. All chrome emoji (💀⚔️🏰⚠️⏸▶↺📜) → `Icon`, all ad-hoc hex → tokens. | ✅ built (device-verify) |
| _(commit 16)_ | **Pre-battle recomposition — "The Muster"** (the §4.2 intro state). Normal mode renders the player's **equipped war kit** — the card back + bounty card fanned and standing on a shelf, the app icon recreated from the player's own gear — by reusing the canonical `MiniBack`/`MiniBounty` (now exported from `Armory.tsx`) + a new `getEquippedKit(theme)` helper, beside the road-ahead (six face-down battle-cards), the Glory Hunt opt-in and Enter Battle. Daily mode shows a sealed quest card on the honor chassis (`script-text` medallion). Motion: bounty-deals-then-back-lands stagger + idle levitate; full-width Cinzel ornate header. Replaces the board (early return) → motion free. | ✅ built (device-verify) |
| _(commit 15)_ | **Between-levels recomposition — "The March"** (§4.3). Rebuilt on `honor.tsx`: left = the field result as a compact honor card (gold/`check-decagram` for a clear, steel/`shield-half-full` for a retreat; running TOTAL SPOILS as the hero number; deal-in + levitate) on a shelf above **the campaign march** — the six battlefields as battle-cards (cleared = sealed deep-green + gold check, next = pulsing gold, rest = face-down), replacing the old plain pips with an icon-true card row; right = perfect/glory chip + next-battlefield preview (mult · beasts · time from `LEVEL_CONFIG`) + Glory Hunt opt-in + Next Battle / Claim Victory primary. Arena → standings ledger + countdown/waiting. Replaces the board (early return) → motion free. | ✅ built (device-verify) |
| _(commit 14)_ | **Shared honor-card chassis + Game-over recomposition — "The Spoils Card"** (§4.4). New `src/ui/honor.tsx` extracts the Hall/Profile card grammar into one canonical chassis — `HonorCard` (deep-green `#1A3D2A` field, engraved double frame, corner runes, Cinzel corner index, crest medallion + breathing halo, deal-in + optional levitate), plus `LedgerRow`/`LedgerSep`/`ForgeBar`/`NamePlate` and the `RANKS`/`TIER`/`withAlpha` helpers. `GameOverScreen` rebuilt landscape-first on it: left = the run as an honor card (outcome-metal trim, crest medallion, SPOILS hero number, all-time-record → goldBright + floating crown + RECORD badge); right = chronicle (async rank shimmer→resolve in a reserved-height slot — closes §4.4.2; dotted-leader ledger rows; §6.5 "one more battle" goal line; Hall-seal confirm; Battle Again primary + Return ghost). Arena → final-rankings ledger. Replaces only the board (early return) so motion is free. **Debt:** Scoreboard/Profile still carry their own card-grammar copies — collapse them onto `honor.tsx` in the migration sweep (there are now 3 copies; this is the canonical one). | ✅ built (device-verify) |
...
**Status:** items 1–5 + 7 ✅ shipped commit 11 (board chrome); item 6 (Battlefield) ✅ shipped commit 12; item 7 (bounty sigil) shipped earlier in §4.2.8 commit 9; the war-bar HUD was further **composed into framed treasury + gauge stations** in commit 18 (the card *field* stays owner-locked — positioning confirmed across devices). The list below is the original spec, kept for reference.
...
### 4.3 Between levels (`betwee-layouts.jpg`, `game-completed-1.jpg` → `BetweenLevelsScreen.tsx`) — **"The March"** (respec'd 2026-06-13, shipped commit 15)

Recomposed on the honor chassis: left column = the field result as a compact honor card (gold trim + `check-decagram` for a clear, steel + `shield-half-full` for a retreat; the running **TOTAL SPOILS** as the hero number; deal-in + idle levitate) standing on a shelf above **the campaign march** — the six battlefields rendered as battle-cards (cleared = sealed deep-green with a gold check, next = pulsing gold, rest = face-down) which *replaces the old plain pips* with an icon-true row of cards; right column = the perfect-clear/glory achievement chip, a next-battlefield **preview** (multiplier · beasts · time pulled from `LEVEL_CONFIG`), the Glory Hunt opt-in, and the Next Battle / Claim Victory primary. Arena keeps a standings ledger + countdown/waiting. Original spec below for reference:
...
### 4.4 Game over / record (`game-completed-2.jpg`, `high-score.jpg` → `GameOverScreen.tsx`) — **"The Spoils Card"** (respec'd 2026-06-13, shipped commit 14)

The original token-list spec (below) read flat beside the elevated Hall/Profile screens — the design lesson applies here too. Respec: the finished run is **sealed into an honor card** in the §4.6.2 podium grammar (via the new shared `src/ui/honor.tsx` chassis), so winning or losing visually rhymes with the leaderboard the player is climbing. Landscape two-column: left **spoils card** = outcome-metal trim (gold victory / steel battle-over / bronze retreat; goldBright + floating crown + RECORD badge for an all-time record), crest medallion (`crown`/`sword-cross`/`shield-half-full`/`trophy-variant`) with breathing halo, deal-in + idle levitate, the SPOILS as the hero number, `#rank` as the card's corner index once it resolves; right **chronicle** = async rank slot that shimmers in a reserved-height row until rank resolves (closes §4.4.2's layout-jump), dotted-leader ledger rows (cleared % · beasts captured · best combo), a §6.5 "one more battle" goal line (NEW PERSONAL BEST struck-state, or "N spoils from your personal best" + bar), Hall-seal confirmation, **Battle Again** primary + Return-to-Castle ghost. Arena → final-rankings ledger + Return primary. `PersonalBestBanner`/`RecordCelebration` overlays retained. Original spec, for reference:
...
### 4.10 Guide / How-to-play (`Introscreen.tsx`) — shipped commit 23

First-run brand splash + the 8-slide how-to-play tutorial (also opened from Home's meta rail). Brought onto the design system without changing the flow: token palette + Cinzel (splash title `font.display`; HOW-TO-PLAY header + slide titles), tutorial emoji → MCI icons in a breathing ring medallion using the **in-game vocabulary** (`restore`=Free Draw, `sack`=Bounty, `lightning-bolt`=Glory Hunt, `image-filter-hdr`=Battlefields, `fire`=Combo, `cards`/`refresh`=match/deck, `trophy-variant`=Prizes), shared `GoldButton` for Next/Enter, token dots + ghost Back with chevrons. The four card-face beasts on the splash stay emoji (§9). Slide copy / phase flow / slide-transition animation / `hasSeenIntro` unchanged.

### 4.11 Lounge (`LoungeScreen.tsx`) — "Venue Hall" — shipped commit 24

The Hookah-Lounge weekly tournament, recomposed as a venue-scaled Hall of Glory (§4.6 grammar). **Joined:** landscape two-column — full-width ornate header (`pillar` venue glyph + Cinzel venue name + dynamic subtitle "N WARRIORS · N DAYS LEFT"); left **shrine** = the player's standing rendered through the shared `HonorCard` (place-metal trim, `#rank` corner index, crest medallion, YOUR SPOILS as the Cinzel hero number; an UNRANKED prompt card when the player hasn't scored this week), deal-in + levitate; right **muster roll** = the week's warriors in the leaderboard grammar (rank numeral + place/tier ring, dotted leader, combo pill, gold score, place metals for the top 3, parchment YOU chip), cascade-in, + a prize strip. Enter Battle (`GoldButton`) + Return below; Leave as a corner danger chip. **Join (not yet in a venue):** centered ornate header + a panel with a Cinzel code input + JOIN `GoldButton`. Off-palette blue `#4FC3F7` → **sage** venue accent (matches Home's lounge dot). `useWindowDimensions` + safe-area insets; service/data/flow unchanged.
...
- **R4 — Streak shield ("Ember Ward") — ✅ SHIPPED 2026-07-02 (commit 40):** one missed day per rolling week auto-forgiven (`emberWardUsedAt` in the user doc; 2-day gap continues the streak, once per 7 days); surfaced as an ember shield on the Home streak chip for a day after firing (a toast isn't reachable — the streak write is fire-and-forget in `saveGameResults`, and `useUserStats` loads once per session; flagged as a known staleness). Shipped with the local-vs-UTC yesterday fix that was wrongly resetting east-of-UTC streaks.
