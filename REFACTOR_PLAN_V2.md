...
- **Tier 4 — small blades + the sound pass** *(doc Tier 5 + the sound replacement)* — **IN PROGRESS.**
  ...
  - `60ca563` — **the sound replacement pass, acquisition half.** Environment can't reach freesound (no token), can't transcode (no ffmpeg), can't audition — so owner chose "curated manifest." Shipped `scripts/sounds-manifest.json` (real, license-checked freesound candidates for all 11 cues; combo ladder = escalating CC0 sword clashes → war-drum boom; long-texture traps excluded) + `scripts/fetch-sounds.js` (self-verifying: refuses NC, skips SA, enforces per-cue max duration, falls back to alternates, writes `ATTRIBUTION.md`, pulls `preview-hq-mp3` so no ffmpeg). Filenames unchanged → **SoundService needs no wiring**; the swap is pure assets once the owner runs it with `FREESOUND_TOKEN` and auditions.
  - **Still open on Tier 4:** owner runs `fetch-sounds.js` + auditions/reorders the combo ladder; the proper-noun quieting (doc Tier 5.1, a copy/style feel pass better done with owner eyes, playtest-gated); `migrateArmoryIfNeeded` retirement (owner yes/no); optional Jest over `src/game/` (edict/scoring/seed determinism). Optional follow-up: the duel game-over currently borrows `playVictory`/`playDefeat` off `isVictory` (field-clear), not off beating the challenger — a tiny verdict-aware tweak if wanted.

**Session checkpoint (2026-07-15):** branch `refactor/v1.4-phase3`, working tree clean, `tsc` green. Tiers 1–3 shipped and closed; Tier 4 partially shipped (three commits above) with the owner-run items listed. No scoring math or blocking graphs were touched anywhere in Tiers 1–4. Owner gates outstanding across tiers: Blaze/Cloud Scheduler (Tier 1), `duels/` Firestore rules (Tier 2), edict-strength playtest (Tier 3), the freesound token + audition (Tier 4), plus the standing Tier 0 release items (board playtest, `bundleRelease` + smoke, the wipe run + broadcast-throttle decision, prize-terms page).*
