...
- **Tier 4 — small blades + the sound pass** *(doc Tier 5 + the sound replacement)* — **SHIPPED.**
  - `3a61ad5` — **stale streak/ward chip** fixed: `useUserStats` now refetches on app-foreground and exposes `refresh()`; `App.tsx` re-pulls on return-to-Home (the hook lives at the root and never remounted — §1.3 carryover debt).
  - `5ead42f` — **repo hygiene**: `reset-users.js` → `scripts/` (require repointed to match `wipe-alltime-scores.js`); dead `assets/sounds/wild.mp3` removed (wild mechanic cut in v1.4, asset unreferenced).
  - `60ca563` — **the sound replacement pass, acquisition half.** Environment can't reach freesound (no token), can't transcode (no ffmpeg), can't audition — so owner chose "curated manifest." `scripts/sounds-manifest.json` (real, license-checked freesound candidates for all 11 cues; combo ladder = escalating CC0 sword clashes → war-drum boom; long-texture traps excluded) + `scripts/fetch-sounds.js` (self-verifying: refuses NC, skips SA, enforces per-cue max duration, falls back to alternates, writes `ATTRIBUTION.md`, pulls `preview-hq-mp3` so no ffmpeg).
  - `0512640` — **duel verdict sound**: game-over now judges a duel on the challenger, not the field-clear — `playDuelWin` (top-tier capture ring + fanfare) / `playDuelLoss` (low double thud), both within the `playAt` variant set.
  - `2071fe4` — **Jest suite** (first tests in the repo): jest-expo (jest pinned to 29), 40 tests over `src/game/` — scoring ladder + cap, match rule, seeded deck/bounty determinism, edict determinism. `npm test`.
  - `b2c961a` — **sound wiring guarantee**: `src/services/soundFiles.ts` (`SOUND_FILES`, the cue→filename source of truth; the manifest deliberately reuses the existing filenames so the swap is zero-code) + a consistency test asserting `SOUND_FILES` == the 11 manifest targets == the mp3s on disk, no orphans. "Drop the files in and it works" is now machine-checked.
  - **Still open on Tier 4 (owner):** run `fetch-sounds.js` + audition/reorder the combo ladder; the proper-noun quieting (doc Tier 5.1, a copy/style feel pass, playtest-gated — not built); `migrateArmoryIfNeeded` retirement (owner yes/no).

- **Tier 5 — Anonymous auth → link-at-first-score** *(doc Tier 1, the sign-in wall — row 10; client slice)* — **SHIPPED.**
  - `92afbd7` — **anon spine**: `onAuthStateChanged` signs in anonymously on first open (no wall, straight to Home as "Wanderer"), falling back to the Google door if the Anonymous provider isn't enabled yet. `isAnon` threads through `App.UserData`. Google users unchanged.
  - `8d9a263` — **capability gating**: anon submits nothing (`saveGameResults` + push prompt gate on `!isAnon`) while all device-local progress (crowns/ghost/local PB/Armory gates/First Victory) still runs; Home's Daily/Arena/Lounge render locked ("Etch your name to enter"); `App.linkWithGoogle` is the shared link flow — `linkWithCredential` (uid preserved, no migration) with the `credential-already-in-use` → `signInWithCredential` veteran-reinstall fallback, then `setUser(null)` remounts AuthScreen to re-resolve heroName.
  - `0faa888` — **the link moment**: the game-over rank slot becomes "ETCH YOUR NAME IN THE HALL" → `onLink`; the rank goal + "Sealed in the Hall" line are replaced for anon ("Kept on this device").
  - `727b0a1` — anon skips the arena-invite + push-token subscriptions (no users doc to hold a token).
  - **Owner gates:** enable the **Anonymous provider** (Firebase console) and confirm Firestore rules are satisfied by anon sessions (`request.auth != null`). **Not built:** deep links (`mythicpeaks://duel/CODE`) remain v2; Profile is still reachable by anon (not in the restriction list — a possible follow-up).

**Session checkpoint (2026-07-15):** branch `refactor/v1.4-phase3`, working tree clean, `tsc` green, 40 Jest tests green. **Tiers 1–5 all shipped and closed.** No scoring math or blocking graphs were touched anywhere in Tiers 1–5. Doc Tiers 5.2–5.4 (proper-noun quieting, migrateArmory retirement) and Tier 6 (parked) remain. Owner gates outstanding across tiers: Anonymous provider + rules (Tier 5), Blaze/Cloud Scheduler + broadcast throttle (Tier 1/§1.2.3), `duels/` Firestore rules (Tier 2), edict-strength playtest (Tier 3), freesound token + audition (Tier 4), plus the standing Tier 0 release items (board playtest, `bundleRelease` + smoke, the wipe run, prize-terms page).*
